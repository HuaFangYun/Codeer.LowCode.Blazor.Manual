...
1. [特徴](#特徴)
2. [利用方法](#利用方法)
3. [こんなプロジェクトにおすすめ](#こんなプロジェクトにおすすめ)
4. [ポトペタで画面作成](#ポトペタで画面作成)
5. [RDBと自在に連携](#RDBと自在に連携)
6. [スクリプトでより自由に](#スクリプトでより自由に)
7. [Excelとの連携のサポート](#Excelとの連携のサポート)
8. [認証・認可](#認証・認可)
9. [こだわりの機能はプロコードで実装](#こだわりの機能はプロコードで実装)
10. [ライセンス情報](#ライセンス情報)
...
## 認証・認可
認証は一般的なCookie認証やAzureActiveDirectoryを使った認証をデフォルトで提供します。その他の認証もカスタマイズで対応可能です。認可に関してはアプリ、画面、データそれぞれでアクセス制御が可能です。
